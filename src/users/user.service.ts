...
import {
  createHash,
  generateAuthToken,
  getDiffBetweenTwoDatesInMinutes,
  handleErrorCatch,
  sendEmail,
} from '../utils/helper';
  ...
  async login(data: any): Promise<any> {
    try {
      ...
      if (isValid) {
        ...
        return {
          message: 'Please complete onboarding process',
        };
      } else {
        throw new HttpException(
          {
  ...
  async fetchAuthUser(data) {
    try {
      const user = await this.userRepo
        ...
        .where(`user.id = :id`, { id: data.tokenData.id })
        .select(['user'])
        .addSelect(['roles'])
        .getOne();
  ...
  async sendResetPasswordToken(email: string) {
    try {
      ...
      return {
        success: true,
        message:
          'An email with your One TIme Password has been sent, Kindly copy the code to reset your password.',
      };
    } catch (err) {
      if (
        err.status === HttpStatus.UNAUTHORIZED ||
